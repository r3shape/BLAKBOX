...
class BOXcontainer(BOXelement):
    ...
    def _layout(self, offset: list[int] = [0, 0]) -> None:
        if not self.get_flag(self.flags.DISPLAY_ROW) and not self.get_flag(self.flags.DISPLAY_LIST) and not self.get_flag(self.flags.DISPLAY_ABSOLUTE):
            BOXlogger.warning("[BOXcontainer] No layout flag set. Defaulting to DISPLAY_ABSOLUTE.")
            self.set_flag(self.flags.DISPLAY_ABSOLUTE)

        r, c = 0, 0
        w, h = 0, 0
        _offset = add_v2(offset, add_v2(self.padding, self.margin))
        for child in self.children.values():
            if self.get_flag(BOXcontainer.flags.DISPLAY_ABSOLUTE):
                return
            elif self.get_flag(BOXcontainer.flags.DISPLAY_ROW):
                if c >= self.wrap_x:
                ...
                c += 1
            elif self.get_flag(BOXcontainer.flags.DISPLAY_LIST):
                if r >= self.wrap_y:
